/**
 * Global Warming — unique chapter mini-games (shared engine).
 * Each activity page calls GlobalGame.boot({ game: '...', ... }).
 */
(function (w) {
  'use strict';
  ...
  function drawRingIcon(x, y, icon, size, ring, flipX) {
    ...
    ctx.fillStyle = ring || 'rgba(20,40,20,0.72)';
    ctx.fill();
  ...
  function drawBg(vs) {
    ...
    g.addColorStop(0, S.cfg.bgTop || '#2e7d32');
    g.addColorStop(1, S.cfg.bgBot || '#1b3a1b');
    S.ctx.fillStyle = g;
    S.ctx.fillRect(0, 0, vs.w, vs.h);
    for (var i = 0; i < 12; i++) {
      S.ctx.fillStyle = 'rgba(255,255,255,' + (0.06 + (i % 3) * 0.04) + ')';
      S.ctx.beginPath();
  ...
  function sortOrder() {
    return (S.cfg.good && S.cfg.good.length) ? S.cfg.good : ['☀️', '🌱', '💨', '🌊', '🌍'];
  }
  ...
  function initZoneSort(vs) {
    ...
    S.state = {
      ...
      step: 0,
      order: order,
      hintText: 'Next: ' + order[0],
    };
  }

  function spawnZoneSort(vs) {
    var order = S.state.order;
    var need = order[S.state.step % order.length];
    if (Math.random() < 0.38) {
      S.state.items.push({
        x: 28 + Math.random() * (vs.w - 56),
        y: -24,
        icon: pick(S.cfg.bad.length ? S.cfg.bad : ['🏭']),
        bad: true,
        speed: 2 + Math.random() * 1.5,
        r: 20,
      });
    } else {
      var icon = Math.random() < 0.62 ? need : pick(order);
      S.state.items.push({
        x: 28 + Math.random() * (vs.w - 56),
        y: -24,
        icon: icon,
  ...
  function loopZoneSort(vs) {
    ...
    S.state.items.forEach(function (it, i) {
      ...
      if (Math.hypot(p.x - it.x, p.y - it.y) < it.r + 18) {
        ...
        if (it.bad) loseLife();
          ...
          addScore(15);
          S.state.step++;
          S.state.hintText = 'Next: ' + order[S.state.step % order.length];
          hud();
        } else loseLife();
    ...
    S.state.items.forEach(function (it) {
      drawRingIcon(it.x, it.y, it.icon, 32, it.bad ? 'rgba(120,20,20,0.75)' : (it.need ? 'rgba(255,193,7,0.5)' : 'rgba(20,60,20,0.78)'), false);
    });
    drawRingIcon(p.x, p.y, p.icon, 36, 'rgba(30,80,30,0.85)', p.vx < -0.4);
  }

  function initSunbeam(vs) {
    S.state = { swimmers: [], spawn: 0, beamX: vs.w * 0.38, beamW: vs.w * 0.24, hintText: 'Tap items in the ☀️ beam!' };
  }
  ...
  function spawnSunbeam(vs) {
    ...
    S.state.swimmers.push({ x: dir > 0 ? -30 : vs.w + 30, y: 50 + Math.random() * (vs.h - 100), icon: pick(pool), bad: bad, dir: dir, speed: 1.8 + Math.random() * 2.2, r: 22 });
  }
  ...
  function loopSunbeam(vs) {
    ...
    drawBg(vs);
    var bx = S.state.beamX, bw = S.state.beamW;
    S.ctx.fillStyle = 'rgba(255,235,59,0.14)';
    S.ctx.fillRect(bx, 0, bw, vs.h);
    S.state.swimmers.forEach(function (s) { drawEmoji(S.ctx, s.icon, s.x, s.y, 30, s.dir < 0); });
  }
  ...
  function tapSunbeam(x, y) {
    for (var i = S.state.swimmers.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(x - s.x, y - s.y) < s.r + 10) {
        ...
        if (!inBeam) return;
        if (s.bad) loseLife(); else addScore(12);
        return;
      }
    }
  }
  ...
  function spawnGlow(vs) {
    ...
    S.state.pulses.push({ x: 40 + Math.random() * (vs.w - 80), y: 40 + Math.random() * (vs.h - 80), icon: bad ? pick(S.cfg.bad) : pick(S.cfg.good), bad: bad, t: 0, life: 120 + Math.random() * 40, r: 24 });
  }

  function loopGlow(vs) {
    S.state.spawn++;
    if (S.state.spawn > 32) { S.state.spawn = 0; spawnGlow(vs); }
    S.state.pulses.forEach(function (p, i) { p.t++; if (p.t > p.life) S.state.pulses.splice(i, 1); });
    drawBg(vs);
    S.state.pulses.forEach(function (p) {
      var phase = p.t / 28, glow = 0.35 + Math.max(0, Math.sin(phase)) * 0.65;
      S.ctx.strokeStyle = 'rgba(129,199,132,' + glow + ')';
  ...
  function tapGlow(x, y) {
    for (var i = S.state.pulses.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(x - p.x, y - p.y) < p.r + 14) {
        var beat = Math.sin(p.t / 28);
        S.state.pulses.splice(i, 1);
        if (beat < 0.55) return;
        if (p.bad) loseLife(); else addScore(10);
        return;
      }
    }
  }
  ...
  function initSonar(vs) {
    S.state = { pings: [], spawn: 0, hintText: 'Tap fast when radar reveals an item!' };
  }

  function spawnSonar(vs) {
    var bad = Math.random() < 0.25;
    S.state.pings.push({ x: 50 + Math.random() * (vs.w - 100), y: 50 + Math.random() * (vs.h - 100), icon: bad ? pick(S.cfg.bad) : pick(S.cfg.good), bad: bad, r: 0, maxR: 55 + Math.random() * 25, reveal: false });
  }
  ...
  function loopSonar(vs) {
    ...
    S.ctx.fillStyle = '#1a2e1a';
    S.ctx.fillRect(0, 0, vs.w, vs.h);
    S.state.pings.forEach(function (p) {
      S.ctx.strokeStyle = 'rgba(129,199,132,' + (0.35 - p.r / p.maxR * 0.3) + ')';
      S.ctx.lineWidth = 2;
      S.ctx.beginPath();
      S.ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2);
      S.ctx.stroke();
      if (p.reveal) { S.ctx.globalAlpha = Math.min(1, (p.maxR - p.r) / 30); drawEmoji(S.ctx, p.icon, p.x, p.y, 30, false); S.ctx.globalAlpha = 1; }
    });
  }
  ...
  function tapSonar(x, y) {
    for (var i = S.state.pings.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(x - p.x, y - p.y) < 30) { S.state.pings.splice(i, 1); if (p.bad) loseLife(); else addScore(12); return; }
    }
  }

  function initTrench(vs) {
    S.state = { player: { x: vs.w * 0.28, y: vs.h / 2, vy: 0, icon: '🐧' }, gaps: [], scroll: 0, spawn: 0, hintText: '⬆️⬇️ Steer through ice gaps!' };
    S.state.gaps.push(makeGap(vs, vs.w + 40));
  }

  function makeGap(vs, x) {
    var gh = 70 + Math.random() * 50;
    return { x: x, gapY: 40 + Math.random() * (vs.h - gh - 80), gapH: gh, w: 36 };
  }

  function loopTrench(vs) {
    var p = S.state.player;
    if (S.keys.u) p.vy = -4.5; else if (S.keys.d) p.vy = 4.5; else p.vy += 0.22;
    p.vy = Math.max(-5.5, Math.min(5.5, p.vy));
    p.y = Math.max(24, Math.min(vs.h - 24, p.y + p.vy));
    S.state.scroll += 2.2;
    S.state.spawn++;
    if (S.state.spawn > 40) { S.state.spawn = 0; S.state.gaps.push(makeGap(vs, vs.w + 20)); }
    S.state.gaps.forEach(function (g, i) { g.x -= 2.2; if (g.x < -60) S.state.gaps.splice(i, 1); });
    var hitWall = false;
    S.state.gaps.forEach(function (g) {
      if (Math.abs(p.x - g.x) < 22 && (p.y - 16 < g.gapY || p.y + 16 > g.gapY + g.gapH)) hitWall = true;
    });
    if (hitWall) loseLife();
    S.score = Math.floor(S.state.scroll / 8);
    hud();
    if (S.cfg.goal && S.score >= S.cfg.goal) end(true);
    S.ctx.fillStyle = '#0a1820';
    S.ctx.fillRect(0, 0, vs.w, vs.h);
    S.state.gaps.forEach(function (g) {
      S.ctx.fillStyle = '#1a3040';
      S.ctx.fillRect(g.x - 18, 0, 36, g.gapY);
      S.ctx.fillRect(g.x - 18, g.gapY + g.gapH, 36, vs.h);
    });
    drawRingIcon(p.x, p.y, p.icon, 28, 'rgba(30,80,100,0.85)', false);
  }

  function initReefMatch(vs) {
    var icons = (S.cfg.good || ['☀️', '💨', '🌊', '🌱']).slice(0, 4);
    var cards = [];
    icons.forEach(function (ic) { cards.push(ic, ic); });
    cards.sort(function () { return Math.random() - 0.5; });
    var cols = 4, rows = 2, pad = 10;
    var cw = (vs.w - pad * (cols + 1)) / cols;
    var ch = (vs.h - pad * (rows + 1) - 20) / rows;
    S.state = {
      cards: cards.map(function (icon, i) {
        return { icon: icon, x: pad + (i % cols) * (cw + pad) + cw / 2, y: 30 + Math.floor(i / cols) * (ch + pad) + ch / 2, w: cw, h: ch, flip: false, matched: false };
      }),
      picked: [], lock: false, hintText: 'Find all matching pairs!',
    };
  }

  function loopReefMatch(vs) {
    S.ctx.fillStyle = '#2e5930';
    S.ctx.fillRect(0, 0, vs.w, vs.h);
    var matched = 0;
    S.state.cards.forEach(function (c) {
      if (c.matched) matched++;
      S.ctx.fillStyle = c.matched ? 'rgba(129,199,132,0.25)' : (c.flip ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.35)');
      S.ctx.strokeStyle = 'rgba(129,199,132,0.5)';
      S.ctx.lineWidth = 2;
      ...
      else { S.ctx.font = 'bold 22px ' + FONT; S.ctx.fillStyle = '#81c784'; S.ctx.fillText('?', c.x, c.y); }
    });
    ...
    for (var i = 0; i < S.state.cards.length; i++) {
      ...
      if (x > c.x - c.w / 2 && x < c.x + c.w / 2 && y > c.y - c.h / 2 && y < c.y + c.h / 2) {
        ...
        if (S.state.picked.length === 2) {
          S.state.lock = true;
          var a = S.state.picked[0], b = S.state.picked[1];
          if (a.icon === b.icon) { a.matched = b.matched = true; addScore(20); S.state.picked = []; S.state.lock = false; }
          else { setTimeout(function () { a.flip = b.flip = false; S.state.picked = []; S.state.lock = false; }, 650); }
        }
        return;
      }
    }
  ...
  function initBreath(vs) {
    S.state = { player: { x: vs.w / 2, y: 60, vy: 0, icon: S.cfg.player || '🏠' }, o2: 100, items: [], hazards: [], spawn: 0, hintText: '⬆️ Move up! ⬇️ Collect eco items' };
  }

  function loopBreath(vs) {
    var p = S.state.player;
    if (S.keys.u) p.vy = -3.5; else if (S.keys.d) p.vy = 3.5; else p.vy *= 0.92;
    p.y = Math.max(36, Math.min(vs.h - 36, p.y + p.vy));
    if (p.y < vs.h * 0.18) S.state.o2 = Math.min(100, S.state.o2 + 2.2);
    else S.state.o2 -= 0.45;
    if (S.state.o2 <= 0) loseLife();
    S.state.spawn++;
    if (S.state.spawn > 36) {
      S.state.spawn = 0;
      if (Math.random() < 0.7) S.state.items.push({ x: 40 + Math.random() * (vs.w - 80), y: vs.h * 0.35 + Math.random() * (vs.h * 0.45), icon: pick(S.cfg.good), r: 18 });
      else S.state.hazards.push({ x: 40 + Math.random() * (vs.w - 80), y: vs.h * 0.35 + Math.random() * (vs.h * 0.45), icon: pick(S.cfg.bad), r: 18 });
    }
    S.state.items.forEach(function (it, i) { if (Math.hypot(p.x - it.x, p.y - it.y) < 28) { S.state.items.splice(i, 1); addScore(10); } });
    S.state.hazards.forEach(function (it, i) { if (Math.hypot(p.x - it.x, p.y - it.y) < 26) { S.state.hazards.splice(i, 1); loseLife(); } });
    drawBg(vs);
    S.ctx.fillStyle = 'rgba(129,212,250,0.2)';
    S.ctx.fillRect(0, 0, vs.w, vs.h * 0.16);
    S.ctx.fillStyle = S.state.o2 > 30 ? '#81c784' : '#ff5252';
    ...
    drawRingIcon(p.x, p.y, p.icon, 34, 'rgba(30,80,30,0.85)', p.vy > 0.5);
  }

  function initSchool(vs) {
    var laneY = [vs.h * 0.45, vs.h * 0.58, vs.h * 0.71];
    S.state = { lane: 1, laneY: laneY, player: { icon: S.cfg.player || '🦋' }, obstacles: [], treats: [], spawn: 0, hintText: '⬅️➡️ Switch lanes — dodge pollution!' };
  }

  function loopSchool(vs) {
    if (S.keys.l && !S.state.keyLock) { S.state.lane = Math.max(0, S.state.lane - 1); S.state.keyLock = true; setTimeout(function () { S.state.keyLock = false; }, 180); }
    if (S.keys.r && !S.state.keyLock) { S.state.lane = Math.min(2, S.state.lane + 1); S.state.keyLock = true; setTimeout(function () { S.state.keyLock = false; }, 180); }
    S.state.spawn++;
    if (S.state.spawn > 30) {
      S.state.spawn = 0;
      var lane = Math.floor(Math.random() * 3);
      if (Math.random() < 0.55) S.state.obstacles.push({ lane: lane, x: vs.w + 20, icon: pick(S.cfg.bad), speed: 3 + Math.random() * 2 });
      else S.state.treats.push({ lane: lane, x: vs.w + 20, icon: pick(S.cfg.good), speed: 2.5 + Math.random() * 1.5 });
    }
    var py = S.state.laneY[S.state.lane], px = vs.w * 0.22;
    S.state.obstacles.forEach(function (o, i) {
      o.x -= o.speed;
      if (o.lane === S.state.lane && Math.abs(o.x - px) < 28) { S.state.obstacles.splice(i, 1); loseLife(); }
      else if (o.x < -30) S.state.obstacles.splice(i, 1);
    });
    S.state.treats.forEach(function (t, i) {
      t.x -= t.speed;
      if (t.lane === S.state.lane && Math.abs(t.x - px) < 26) { S.state.treats.splice(i, 1); addScore(8); }
      else if (t.x < -30) S.state.treats.splice(i, 1);
    });
    drawBg(vs);
    S.state.laneY.forEach(function (ly, idx) {
      S.ctx.strokeStyle = idx === S.state.lane ? 'rgba(129,199,132,0.35)' : 'rgba(255,255,255,0.06)';
      S.ctx.lineWidth = idx === S.state.lane ? 2 : 1;
      S.ctx.beginPath(); S.ctx.moveTo(0, ly); S.ctx.lineTo(vs.w, ly); S.ctx.stroke();
    });
    S.state.obstacles.forEach(function (o) { drawEmoji(S.ctx, o.icon, o.x, S.state.laneY[o.lane], 28, true); });
    S.state.treats.forEach(function (t) { drawEmoji(S.ctx, t.icon, t.x, S.state.laneY[t.lane], 24, true); });
    drawRingIcon(px, py, S.state.player.icon, 32, 'rgba(30,80,30,0.85)', false);
  }
  ...
  function bindControls(kind) {
    var L = $('ogLeftBtn'), R = $('ogRightBtn'), U = $('ogUpBtn'), D = $('ogDownBtn');
    ...
    if (kind === 'lr' || kind === 'school-run') { bindBtn(L, 'l'); bindBtn(R, 'r'); }
    if (kind === 'ud') { bindBtn(U, 'u'); bindBtn(D, 'd'); }
