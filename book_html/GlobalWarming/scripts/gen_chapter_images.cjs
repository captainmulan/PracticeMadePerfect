...
const OUT = path.join(DIR, "_global-chapter-images.js");

const sandbox = { window: {}, console };
vm.runInNewContext(fs.readFileSync(path.join(DIR, "_global-data.js"), "utf8"), sandbox);
const IDS = sandbox.window.GLOBAL_CHAPTERS.map((c) => c.id);
...
(function (w) {
  w.GLOBAL_CHAPTER_IMAGES = ${JSON.stringify(images)};
  ...
  w.GlobalChapterImage = {
    render: function (id, slot, title) {
      var ch = w.GLOBAL_CHAPTER_IMAGES[id];
