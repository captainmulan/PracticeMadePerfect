/**
 * Continents Adventure — unique chapter mini-games (shared engine).
 * Each activity page calls ContinentGame.boot({ game: '...', ... }).
 */
(function (w) {
  'use strict';
  ...
  function spawnZoneSort(vs) {
    var need = zoneOrder()[S.state.step % zoneOrder().length];
    if (Math.random() < 0.38) {
      S.state.items.push({
      ...
      var icon = Math.random() < 0.62 ? need : pick(zoneOrder());
      S.state.items.push({
        x: 28 + Math.random() * (vs.w - 56),
        y: -24,
        icon: icon,
    ...
    S.state.items.forEach(function (it, i) {
      ...
      if (Math.hypot(p.x - it.x, p.y - it.y) < it.r + 18) {
        ...
        if (it.bad) loseLife();
          ...
          addScore(15);
          S.state.step++;
          S.state.hintText = 'Next: ' + zoneOrder()[S.state.step % zoneOrder().length];
          hud();
        } else loseLife();
