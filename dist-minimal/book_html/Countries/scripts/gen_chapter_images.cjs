#!/usr/bin/env node
/* Embed chapter PNGs as data URIs */
const fs = require("fs");
...
const OUT = path.join(DIR, "_countries-chapter-images.js");

const sandbox = { window: {}, console };
vm.runInNewContext(fs.readFileSync(path.join(DIR, "_countries-data.js"), "utf8"), sandbox);
const IDS = sandbox.window.COUNTRIES_CHAPTERS.map((c) => c.id);
...
(function (w) {
  w.COUNTRIES_CHAPTER_IMAGES = ${JSON.stringify(images)};
  ...
  w.CountriesChapterImage = {
    render: function (id, slot, title) {
      var ch = w.COUNTRIES_CHAPTER_IMAGES[id];
