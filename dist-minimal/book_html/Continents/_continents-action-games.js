/**
 * Continents Adventure — 3 quality action games (mid-book + outro).
...
(function (w) {
  ...
  function injectStyles() {
    if ($("continent-action-game-styles")) return;
    var s = document.createElement("style");
    s.id = "continent-action-game-styles";
    s.textContent =
      "html:has(body.continent-book-game),html:has(body.continent-book-game){height:100%!important;overflow:hidden!important;}" +
      "body.continent-book-game{overflow:hidden!important;height:100%!important;min-height:100%!important;margin:0;}" +
      "body.continent-book-game .game-shell{max-width:980px;margin:0 auto;padding:8px 10px;display:flex;flex-direction:column;height:100%;min-height:0;}" +
      "body.continent-book-game .stage-wrap{flex:1 1 auto;min-height:0;display:flex;}" +
      "body.continent-book-game .action-stage{position:relative;flex:1 1 auto;min-height:min(52vh,420px);border:2px solid rgba(100,255,218,.45);border-radius:16px;overflow:hidden;background:#1b2e0a;touch-action:none;}" +
      "body.continent-book-game .action-stage canvas{display:block;width:100%;height:100%;}" +
      "body.continent-book-game .action-overlay{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(4,20,36,.92);text-align:center;padding:18px;z-index:5;}" +
      "body.continent-book-game .action-overlay h3{color:#aed581;font-size:22px;margin-bottom:8px;}" +
      "body.continent-book-game .action-overlay p{color:#fff3e0;font-size:14px;line-height:1.55;margin-bottom:14px;max-width:360px;}" +
      "body.continent-book-game .game-start-btn{padding:11px 24px;border:none;border-radius:12px;background:linear-gradient(135deg,#aed581,#689f38);color:#1b2e0a;font-size:15px;font-weight:bold;cursor:pointer;}" +
      "body.continent-book-game .action-controls{display:flex;gap:10px;justify-content:center;margin-top:8px;flex-shrink:0;}" +
      "body.continent-book-game .action-btn{min-width:72px;padding:14px 18px;border-radius:12px;border:2px solid rgba(100,255,218,.4);background:rgba(100,255,218,.12);font-size:22px;cursor:pointer;touch-action:manipulation;}" +
      "body.continent-book-game .game-meta{text-align:center;color:#ffe082;font-size:14px;font-weight:bold;margin:6px 0;}" +
      "body.continent-book-game .badge-box{text-align:center;margin-top:8px;padding:10px;border-radius:12px;background:rgba(255,235,59,.12);color:#ffeb3b;font-size:13px;}" +
      "@media(orientation:landscape){body.continent-book-game .game-shell{max-width:none;padding:6px 12px;}" +
      "body.continent-book-game .action-stage{min-height:0!important;max-height:none!important;}" +
      "body.continent-book-game .page-head{margin-bottom:4px!important;}" +
      "body.continent-book-game .page-head h1{font-size:20px!important;}" +
      "body.continent-book-game .page-head .lead{font-size:12px!important;}}" +
      "@media(max-width:639px){body.continent-book-game .action-stage{min-height:min(46vh,380px);}}";
    document.head.appendChild(s);
  }

  function fitFrame() {
    if (!document.body.classList.contains("continent-book-game")) return;
  ...
  function drawEarthBg(ctx, W, H, top, bot) {
    ...
    g.addColorStop(0, top || "#33691e");
    g.addColorStop(1, bot || "#3e2723");
    ctx.fillStyle = g;
  ...
  /* ── Continent Trek (mid-book): stomp-style lane runner through ocean zones ── */
  function bootZoneDive(opts) {
    ...
    function endGame(won) {
      ...
      if (overlay) {
        ...
        overlay.querySelector("h3").textContent = won ? "🏆 " + (opts.badge || "Continent Trek Hero") + "!" : "💫 Try again!";
        overlay.querySelector("p").textContent = won
          ? "You cleared all " + maxLevel + " continent levels!"
    ...
    function loop() {
      ...
      drawEarthBg(ctx, W, H, ["#558b2f", "#f9a825", "#5d4037", "#1565c0", "#90a4ae", "#eceff1", "#ef6c00"][zi], ["#33691e", "#ef6c00", "#3e2723", "#0d47a1", "#546e7a", "#b0bec5", "#bf360c"][zi]);
      ...
      var pIcon = (w.ContinentPlayer && w.ContinentPlayer.getCharacter()) || player.icon;
  ...
  function bootTreasureRush(opts) {
    ...
    function endGame(won) {
      ...
      if (overlay) {
        overlay.style.display = "flex";
        overlay.querySelector("h3").textContent = won ? "🏆 Globe Master!" : "💫 Keep Exploring!";
    ...
    function loop() {
      ...
      drawEarthBg(ctx, W, H, "#0a2848", "#031525");
      ...
      var pIcon = (w.ContinentPlayer && w.ContinentPlayer.getCharacter()) || "🧭";
  ...
  w.ContinentActionGame = {
    boot: function (opts) {
      injectStyles();
      fitFrame();
      w.addEventListener("resize", fitFrame);
      if (opts.gameType === "continent-trek") bootZoneDive(opts);
      else if (opts.gameType === "globe-rush") bootTreasureRush(opts);
