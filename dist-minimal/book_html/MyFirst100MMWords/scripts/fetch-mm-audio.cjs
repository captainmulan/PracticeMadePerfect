/* Download Myanmar TTS MP3s for offline/local playback (Google translate_tts). */
const fs = require("fs");
...
function ttsUrl(text, mirror) {
  ...
  if (mirror === 0) {
    return "https://translate.googleapis.com/translate_tts?ie=UTF-8&client=gtx&tl=my&q=" + q;
  }
  return "https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=my&q=" + q;
}
...
async function main() {
  ...
  console.log("Unique MM strings:", list.length);
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    try {
      ...
      fail++;
      console.log("FAIL " + text.slice(0, 40) + " :: " + e.message);
    }
