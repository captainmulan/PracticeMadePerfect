...
if (!res.ok) process.exitCode = 2; else process.exitCode = 0;

// Run checklist / keyword verification (mirror verifyTaskAgainstCode rules)
function getExecutableCodeLocal(code) {
  return code
    ...
    .filter((line) => {
      ...
      if (task.type === 'sql') return !trimmed.startsWith('--');
...
const checklistResults = runChecklist(task, code);
console.log('Checklist results:');
task.checklist.forEach((item, idx) => {
  console.log(`- [${checklistResults[idx] ? 'x' : ' '}] ${item}`);
});
