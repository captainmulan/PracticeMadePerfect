"""Batch-import every new PDF under ToUpload. One book per PDF. Not app source."""
...
def book_id_simple(stem: str) -> str:
...
def book_id_from_stem(stem: str, dirs: list[str] | None = None) -> str:
    cleaned = clean_stem(stem)
    parts = [p.strip() for p in re.split(r"\s+-\s+", cleaned) if p.strip()]
    series = ""
    for folder in dirs or []:
        if folder.lower() not in {"kid", "other", "comic", "unknown"}:
            series = folder
    if len(parts) >= 2:
        slug = book_id_simple(parts[-1])
        if series:
            prefix = book_id_simple(series)
            if prefix and not slug.startswith(prefix + "-") and slug != prefix:
                slug = f"{prefix}-{slug}"
        return slug
    return book_id_simple(cleaned)


def title_from_stem(stem: str, dirs: list[str] | None = None) -> str:
    cleaned = clean_stem(stem)
    parts = [p.strip() for p in re.split(r"\s+-\s+", cleaned) if p.strip()]
    if len(parts) >= 2:
        last = readable(parts[-1])
        series = next((d for d in (dirs or []) if d.lower() not in {"kid", "other", "comic"}), None)
        if series and not last.lower().startswith(series.lower()):
            return f"{series} - {last}"
        return last
...
def author_from_sources(doc: pymupdf.Document, folder_author: str, stem: str) -> str:
    cleaned = clean_stem(stem)
    parts = [p.strip() for p in re.split(r"\s+-\s+", cleaned) if p.strip()]
    if len(parts) >= 2:
        return canonical_author(readable(parts[0]))
    meta = doc.metadata or {}
    from_pdf = (meta.get("author") or "").strip()
    if from_pdf and not from_pdf.lower().startswith("unknown"):
        return canonical_author(from_pdf)
    if folder_author.lower().startswith("unknown"):
        return "Unknown"
    return canonical_author(folder_author)
...
def make_course(book: dict, page_count: int, author: str, cover: str, course_index: int) -> dict:
    ...
    return {
        "id": bid,
        "title": book["title"],
        "description": f"{book['title']}. {cat2 or cat1 or 'Book'}.",
        "isPublished": True,
        "authorName": author,
        "color": "#2563eb",
...
def main() -> None:
    ...
    for src in discover():
        ...
        bid = book_id_from_stem(src.stem, dirs)
        print("import", bid, "/", "/".join(dirs))
        ...
        author = author_from_sources(doc, folder_author, src.stem)
        doc.close()
        cover = save_cover(dest, bid)
        prepared.append(
            {
                "id": bid,
                "title": title_from_stem(src.stem, dirs),
                "dirs": dirs,
                "pages": pages,
                "author": author,
                "cover": cover,
        ...
        results.append((bid, "imported", f"{pages}p {author} {original / 1e6:.2f}->{size / 1e6:.2f}MB {how}"))
